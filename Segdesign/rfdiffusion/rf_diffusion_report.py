...
def parse_args():
    ...
    parser.add_argument('--input_pdb', type=str, help='Path of the protein file')
...
def report_csv(input_pdb, pdb_folder, dssp_csv_folder, start_res, end_res, ss, threshold, filter_folder, out_folder):
    """
    生成项目的摘要报告
    param pdb_folder:
    ...
    original_protein_file = os.path.basename(input_pdb)
    original_protein_name = original_protein_file.rsplit(".", 1)[0]
    matched_pdb_files.insert(0, original_protein_file)
    file_name_list.insert(0, original_protein_name)
    pdb_files_path_list.insert(0, input_pdb)

    hmmer_out = input_pdb.rsplit('/', 2)[0]
    rf_out = dssp_csv_folder.rsplit('/', 1)[0]
    original_protein_csv = os.path.join(hmmer_out, 'target_sequence', f'{original_protein_name}.csv')
    if not os.path.exists(original_protein_csv):
        protein_dssp = os.path.join(rf_out, 'dssp_file', f'{original_protein_name}.dssp')
        protein_dssp_csv = os.path.join(dssp_csv_folder, f'{original_protein_name}.csv')
        if not os.path.exists(protein_dssp_csv):
            run_dssp(input_pdb, protein_dssp)
            dssp_to_csv(protein_dssp, protein_dssp_csv)
            original_protein_csv = protein_dssp_csv
            csv_files_path_list.insert(0, original_protein_csv)
    ...
    # 封装待校验的列表（名称: 实际列表），与循环zip的顺序一致
    print('pdb_files_path_list:', pdb_files_path_list)
    print('pdb_files_path_list长度：', len(pdb_files_path_list))
    print('Length of pdb_files_path_list:', len(pdb_files_path_list))
    print('csv_files_path_list:',csv_files_path_list)
    print('csv_files_path_list长度：', len(csv_files_path_list))
    print('Length of csv_files_path_list:', len(csv_files_path_list))
    print('file_name_list:',file_name_list)
    print('file_name_list长度：', len(file_name_list))
    print('Length of file_name_list:', len(file_name_list))
    print('matched_pdb_files:',matched_pdb_files)
    print('matched_pdb_files长度：', len(matched_pdb_files))
    print('Length of matched_pdb_files:', len(matched_pdb_files))
    check_lists = {
        "pdb_files_path_list(PDB路径列表)": pdb_files_path_list,
        "csv_files_path_list(CSV路径列表)": csv_files_path_list,
        "file_name_list(索引名称列表)": file_name_list,
        "matched_pdb_files(PDB文件名列表)": matched_pdb_files
    }
    # 获取基准长度（以第一个列表为准）
    base_len = len(next(iter(check_lists.values())))
    # 遍历校验所有列表长度
    mismatch = []
    for list_name, lst in check_lists.items():
        if len(lst) != base_len:
            mismatch.append(f"{list_name}: {len(lst)}")
    # 存在长度不匹配则抛出明确错误
    if mismatch:
        raise ValueError(
            f"原始蛋白插入后，关键列表长度不一致！基准长度（第一个列表）：{base_len}，不匹配列表：{'; '.join(mismatch)}\n"
            f"请检查：1.PDB文件夹与CSV文件夹的文件数量是否一致；2.原始蛋白插入操作是否出错；3.文件名是否一一对应。"
        )
    else:
        print('已检测，pdb_files_path_list, csv_files_path_list, file_name_list, matched_pdb_files长度一致')
        print('Checked, pdb_files_path_list, csv_files_path_list, file_name_list, matched_pdb_files have consistent lengths')
    num_f = 0
    n = 0
    for pdb_file_path, csv_file_path, file_name, file in zip(pdb_files_path_list, csv_files_path_list, file_name_list, matched_pdb_files):
        print('骨架id：', file_name)
        print('Backbone id:', file_name)
        ...
        print(f'设计区域二级结构(ss8)：{design_seq8}')
        print(f'Designed region secondary structure (ss8): {design_seq8}')
        design_ss8.append(design_seq8)
        design_ss3.append(design_seq)
        for key, value in dict_prop.items():
            if key == 'H':
                print(f'螺旋占比：{value:.2%}')
                print(f'Helix proportion: {value:.2%}')
                h_prop.append(value)
            if key == 'E':
                print(f'折叠占比：{value:.2%}')
                print(f'Strand proportion: {value:.2%}')
                e_prop.append(value)
            if key == 'C':
                print(f'无序卷曲占比：{value:.2%}')
                print(f'Coil proportion: {value:.2%}')
                c_prop.append(value)
        ...
        if n == 0:
            ...
            if ss == 'coil':
                if c_prop[-1] < threshold:
                    ...
                    success_backbone_l.append(os.path.join(filter_folder, file))
        print(f'目标结构类型: {ss}')
        print(f'Target structure type: {ss}')
        print(f'筛选阈值： {threshold}')
        print(f'Filtering threshold: {threshold}')
        if success_l[-1] != 'Yes':
            print(f'是否通过筛选：No\n')
            print(f'Passed filtering: No\n')
        else:
            print(f'是否通过筛选：Yes\n')
            print(f'Passed filtering: Yes\n')
...
if __name__ == '__main__':
    ...
    if final_report_folder is not None:
    ...
    if nf == 0:
        print(f"没有蛋白质骨架通过筛选，不进行后续操作。")
        print(f"No protein backbone passed filtering, no further operations will be performed.")
        sys.exit(100)
